...
def resolve_data_file():
    env_path = os.environ.get("COUNTRY_DATA_FILE")
    candidate_files = []
    expected_names = {
        "country-data.csv",
        "country_data.csv",
    ...
    if env_path:
        candidate_files.append(Path(env_path).expanduser())

    candidate_files.extend(
        [
            BASE_DIR / "Country-data.csv",
            BASE_DIR / "country-data.csv",
            BASE_DIR / "Country_data.csv",
            BASE_DIR / "country_data.csv",
        ]
    )

    project_csv_files = sorted(
        csv_file
        for csv_file in BASE_DIR.rglob("*.csv")
        if OUTPUT_DIR not in csv_file.parents
    )

    for csv_file in project_csv_files:
        if csv_file.name.lower() in expected_names and csv_file not in candidate_files:
...
def show_cached_results():
    ...
    result_file = OUTPUT_DIR / "help_international_submission.csv"

    print("\nCountry-data.csv was not found in the project folder.")
    print("Set the COUNTRY_DATA_FILE environment variable or place the CSV next to model.py to rerun the full analysis.")

    if summary_file.exists():
        print("\nUsing the previously generated summary from .dist/model_summary.txt:\n")
        print(summary_file.read_text(encoding="utf-8"))
        return

    if result_file.exists():
        print("\nUsing the previously generated recommendations from .dist/help_international_submission.csv:\n")
        print(pd.read_csv(result_file))
        return

    print("\nNo cached outputs were found in .dist, so the analysis cannot continue without the raw dataset.")
...
print("Using dataset:", DATA_FILE if DATA_FILE is not None else "not found")
